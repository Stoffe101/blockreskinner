...
- **Skulls & Heads** — vanilla mob heads: Skeleton Skull, Wither Skeleton Skull, Zombie Head, Creeper Head, Piglin Head, and Dragon Head. Floor variants come in four cardinal rotations and wall variants in four facings. Heads have no chunk model, so a dedicated client-only renderer draws the vanilla skull model at skinned positions — no block entity is ever created and the real block is untouched.
- **Player Heads** — open the GUI, pick Skulls & Heads, select Player Head, type a player name (3-16 characters: letters, digits, underscore), press Enter to resolve, pick a rotation, and Apply. Only the name and rotation are stored and synced; every client resolves the profile and skin texture through the vanilla player skin cache (asynchronously, online when available). Until the profile resolves — or if it fails — the head renders with the default player skin and is labeled unresolved in the GUI and Jade. Floor player heads only for now; wall-mounted player heads are not supported yet. The real block still behaves as itself.
